<?php
echo ' <div class="row slideInDown">
<div class="col-md-4 py-2 bg-white rounded-lg shadow-sm">

	<div class="card">
		<div class="card-content">
			<div class="card-header">
				<h5 class="my-3">CREATE PRODUCTS<i class="fa fa-circle-o-notch fa-spin close" style="font-size:18px"></i></h5>
			</div>
			<div class="card-body">
				<form>
					<select class="form-control mb-3" style="border:none;background:#f9f9f9">
						<option>Choose category</option>
					</select>

					<select class="form-control mb-3" style="border:none;background:#f9f9f9">
						<option>Choose brands</option>
					</select>

					<input type="text" class="form-control mb-3" placeholder="Nokia 1100" style="border:none;background:#f9f9f9">
					<button class="btn btn-primary mb-3"><i class="fa fa-plus"></i> Add fields</button>

					<button class="btn btn-danger mb-3">CREATE</button>
				</form>
			</div>
		</div>
	</div>
</div>
<div class="col-md-2">

</div>
<div class="col-md-6 bg-white rounded-lg shadow-sm">
	<div class="card">
		<div class="card-content">
			<div class="card-header">
				<h5 class="my-3">PRODUCTS LIST</h5>
			</div>
			<div class="card-body"></div>
		</div>
	</div>


</div>
</div>';
